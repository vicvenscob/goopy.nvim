...
function M.setup(opts)
  ...
  if not ok then
    vim.notify("goopy: config.setup failed: " .. tostring(err), vim.log.levels.ERROR)
  end

  paths.ensure_dirs()

  -- Register commands FIRST so :Goopy* is always available, even if
  -- parsing/loading plugin specs below errors out.
  local cmd_ok, cmd_err = pcall(require("goopy.commands").register, M)
  if not cmd_ok then
    vim.notify("goopy: failed to register commands: " .. tostring(cmd_err), vim.log.levels.ERROR)
  end

  local raw_specs = opts.plugins or opts.specs or {}

  local parse_ok, parsed_or_err = pcall(config.parse, raw_specs)
  if not parse_ok then
    vim.notify("goopy: failed to parse plugin specs: " .. tostring(parsed_or_err), vim.log.levels.ERROR)
    M.plugins = {}
    return
  end
  M.plugins = parsed_or_err

  -- detect dependency cycles early
  local sort_ok, sort_result1, sort_result2 = pcall(graph.sort, M.plugins)
  if sort_ok then
    local cycle = sort_result2
    if cycle then
      logger.error("dependency cycle detected: " .. table.concat(cycle, ", "))
    end
  else
    logger.error("goopy: graph.sort failed: " .. tostring(sort_result1))
  end

  state.register_all(M.plugins)
  M._wrap_loader_for_profiling()

  local trig_ok, trig_err = pcall(loader.setup_triggers, M.plugins)
  if not trig_ok then
    vim.notify("goopy: failed to set up plugin triggers: " .. tostring(trig_err), vim.log.levels.ERROR)
  end
end
